...
second_chance_to_dma:
	...
	if (hwif->mate && hwif->mate->dma_base) {
		...
		dma_base = (unsigned long)ioremap(pci_resource_start(dev, 4), pci_resource_len(dev, 4));
#else
...
#if defined(CONFIG_ARCH_STR9100) || defined(CONFIG_ARCH_STR8100)
		ctl  = (unsigned long)ioremap(pci_resource_start(dev, 2*port+1), pci_resource_len(dev, 2*port+1));
		base = (unsigned long)ioremap(pci_resource_start(dev, 2*port), pci_resource_len(dev, 2*port));
#else	
