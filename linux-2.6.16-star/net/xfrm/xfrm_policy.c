...
/* TF1: CONFIG_VIPSEC */
int (*ipsec_dst_lookup_hook)(struct dst_entry **dst_p, struct flowi *fl,
                             struct sock *sk, int flags) = NULL;
int (*ipsec_policy_check_hook) (struct sock *sk, int dir, 
                    struct sk_buff *skb, unsigned short family) = NULL;
EXPORT_SYMBOL(ipsec_dst_lookup_hook);
EXPORT_SYMBOL(ipsec_policy_check_hook);
/* TF1: CONFIG_VIPSEC */
...
/* TF1: CONFIG_VIPSEC */
    /* Function which does all the IPSec input processing */
    if (ipsec_dst_lookup_hook != NULL) {
        return (*ipsec_dst_lookup_hook)(dst_p, fl, sk, flags);
    }
/* TF1: CONFIG_VIPSEC */
