 ...
 *
 * Version:	$Id: udp.c,v 1.1.6.3 2007/12/13 14:35:28 suheil Exp $
 *
 * Authors:	Ross Biro
...
/* TF1: CONFIG_VIPSEC */
int (* ipsec_udp_decap_hook) (struct sock *, struct sk_buff *) = 0;
EXPORT_SYMBOL(ipsec_udp_decap_hook);
/* TF1: CONFIG_VIPSEC */
...
static int udp_queue_rcv_skb(struct sock * sk, struct sk_buff *skb)
{
	...
	if (up->encap_type) {
		...
		/* TF1: CONFIG_VIPSEC */
        if (ipsec_udp_decap_hook != NULL) {
            int ret = (*ipsec_udp_decap_hook)(sk, skb);
            if (ret >= 0)
                return ret;
        }
/* TF1: CONFIG_VIPSEC */
