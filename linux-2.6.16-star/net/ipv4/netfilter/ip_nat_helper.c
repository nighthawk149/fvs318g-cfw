...
#if CONFIG_STR9100_SHNAT
// Mark for Port Mode Porting
	if (star9100_shnat_hook_ready) {
		int hnat_hash;
		struct ip_conntrack *ct2;
		int dir = exp->dir;

#if 1
		if (ct->master) {
			if (ntohs(ct->master->tuplehash[0].tuple.dst.u.all) == 21) {
			// tuple.dst.u.all =20 , PORT MODE (WAN->LAN and LAN->WAN)
#if 1
				if (((ntohs(ct->tuplehash[1].tuple.dst.u.all) == 20)&&
					(ct->master->tuplehash[0].tuple.dst.ip == ct->tuplehash[0].tuple.dst.ip))||
					((ntohs(ct->master->tuplehash[0].tuple.dst.u.all) == 21)&&
					(ct->tuplehash[0].tuple.dst.ip == ct->master->tuplehash[0].tuple.dst.ip)&&
					(ct->tuplehash[1].tuple.dst.ip != ct->master->tuplehash[1].tuple.dst.ip))) {
		
					DEBUGP(" LAN => WAN, Passive Mode and WAN->LAN Port Mode \n");
#endif
					hnat_hash =
						star9100_shnat_preadd_hnatable_hook(ct->tuplehash[dir].tuple.src.ip,
							ct->tuplehash[dir].tuple.src.u.tcp.port,
							ct->tuplehash[dir].tuple.dst.u.tcp.port,
							IPPROTO_TCP);
					hnat_hash = hnat_hash + HNAT_SOURCE_PORT_BASE;
					if ((hnat_hash>=0)&&(star9100_shnat_check_ftponly_enable_hook()==0)) {
						range.min.all = htons(hnat_hash);
						range.max.all = range.min.all;
						range.flags |= IP_NAT_RANGE_PROTO_SPECIFIED;
					} else {
						if (star9100_shnat_check_shnat_enable_hook()==1) {
							range.min.all = htons(0);
							range.max.all = htons(HNAT_SOURCE_PORT_BASE-1);
						}
					}
				}
			}
...
#endif
#if 0
		printk("%s:[tuplehash[%d] ",__FUNCTION__,dir);
...
#if 0
			printk("%s:[th_master[%d] ",__FUNCTION__,dir);
			DUMP_TUPLE2(&ct2->tuplehash[dir].tuple);
			printk("%s:[th_master[%d] ",__FUNCTION__,!dir);
			DUMP_TUPLE2(&ct2->tuplehash[!dir].tuple);
#endif
		}
