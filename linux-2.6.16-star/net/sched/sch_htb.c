 ...
 *
 * $Id: sch_htb.c,v 1.1.2.1 2007/12/12 13:20:39 suheil Exp $
 */
...
#ifdef CONFIG_IP_NF_TARGET_BWPROF
#include <linux/netfilter_ipv4/ip_conntrack.h>
#include <linux/netfilter_ipv4/ipt_BWPROF.h>
#endif /* CONFIG_IP_NF_TARGET_BWPROF */
...
#define HTB_DEBUG 0	/* compile debugging support (activated by tc tool) */
...
#ifdef CONFIG_IP_NF_TARGET_BWPROF
            /* 
             * If the packet has a valid tc_index, but there is no class for
             * that tc_index, invalidate the tc_index for the connection
             * stream, so that subsequent packets of the same stream force the
             * creation of the class. This situation can happen with ppp-like
             * interfaces when they disappear and re-appear.
             */ 
        if (unlikely(skb->tc_index != BWPROF_NO_TC_INDEX ))
            {
             extern int bwprof_invalidate_tc_index(struct sk_buff *, char *);
             bwprof_invalidate_tc_index(skb, sch->dev->name);
            }
#endif /* CONFIG_IP_NF_TARGET_BWPROF */
