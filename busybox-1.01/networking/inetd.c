...
--------------------
01a,20Feb07,sag added suppport for passing source ip address to the login
            program
*/   
		...
		for (sep = servtab; n && sep; sep = sep->se_next) {
			...
			if (sep->se_fd != -1 && FD_ISSET(sep->se_fd, &readable)) {
				...
				if (!sep->se_wait && sep->se_socktype == SOCK_STREAM) {
                    ...
                    salen = sizeof(sck_in);
                    ctrl = accept(sep->se_fd,(struct sockaddr *)&sck_in, 
                                  &salen); 
				...
				if (pid == 0) {
...
#endif
					{
						...
						sigaction(SIGPIPE, &sa, NULL);
                                               ...
                                               /* set env variable REMOTE_ADDR to the remote addr */
                	 	               env = setenv("REMOTE_ADDR", 
                                                            inet_ntoa(sck_in.sin_addr), 1);
